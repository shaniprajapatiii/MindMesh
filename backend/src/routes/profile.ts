...
router.get('/me', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const userXp = (await mongoDb.user.findUnique({ where: { id: req.user!.id }, select: { xp: true } }))?.xp || 0;
    const rank = await mongoDb.user.count({ where: { xp: { gt: userXp }, showOnLeader: true } }) + 1;
...
router.put('/me', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const allowed = ['name', 'username', 'bio', 'location', 'college', 'github', 'twitter', 'linkedin', 'isPublic', 'leetcodeId', 'codeforcesId', 'codechefId', 'gfgId', 'githubId'];
    const data: Record<string, any> = {};
    allowed.forEach(k => { if (req.body[k] !== undefined) data[k] = req.body[k]; });
    const user = await mongoDb.user.update({ where: { id: req.user!.id }, data });
    res.json(user);
...
router.get('/:username', optionalAuth as any, async (req: Request, res: Response) => {
  try {
    ...
    const totalSolved = await mongoDb.userProblemStatus.count({ where: { userId: user.id, status: 'solved' } });
