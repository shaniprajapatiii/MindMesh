...
router.put('/', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const allowed = ['name','bio','location','college','leetcodeId','codeforcesId','codechefId','gfgId','githubId','atcoderId','editorTheme','defaultLanguage','fontSize','isPublic','showOnLeader'];
...
router.get('/export', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const [user, notes, statuses, submissions] = await Promise.all([
      ...
      prisma.note.findMany({ where: { userId: req.user!.id } }),
      prisma.userProblemStatus.findMany({ where: { userId: req.user!.id } }),
      prisma.submission.findMany({ where: { userId: req.user!.id }, take: 100 }),
    ]);
