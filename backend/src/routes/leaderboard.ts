...
router.get('/', optionalAuth as any, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const users = await mongoDb.user.findMany({ where, select: { id: true, name: true, username: true, avatar: true, xp: true, streak: true, cfRating: true, leetcodeSolved: true, college: true }, orderBy: { xp: 'desc' }, skip, take: limit });
    const solvedCounts = await mongoDb.userProblemStatus.groupBy({ by: ['userId'], where: { userId: { in: users.map(u => u.id) }, status: 'solved' }, _count: true });
    ...
    let myRank = null;
    if (req.user) {
      const myUser = await mongoDb.user.findUnique({ where: { id: req.user.id }, select: { xp: true, streak: true } });
      const myPos = await mongoDb.user.count({ where: { xp: { gt: myUser?.xp || 0 }, showOnLeader: true } }) + 1;
      const totalUsers = await mongoDb.user.count({ where: { showOnLeader: true } });
      const mySolved = await mongoDb.userProblemStatus.count({ where: { userId: req.user.id, status: 'solved' } });
    ...
    res.json({ rankings, myRank, total: await mongoDb.user.count({ where }) });
